...
AIScreenshotAssi is a global, floating AI assistant for macOS: type a trigger in any app to open a lightweight capsule, press Enter, and it generates text you can immediately use—then returns focus back to your app.
...
- `AISCREENSHOTASSI_LLM_KIND` (usually `openai_compat`)
- `AISCREENSHOTASSI_LLM_MODEL`
- `AISCREENSHOTASSI_LLM_API_KEY`
- `AISCREENSHOTASSI_LLM_BASE_URL` (if you use a proxy / OpenAI-compatible gateway)
...
AIScreenshotAssi needs system permissions to support global summon and input injection:
...
Go to: System Settings → Privacy & Security → enable AIScreenshotAssi in the relevant sections.

## Usage

1) In any app, type the trigger: `//`
2) Type what you want and press Enter
3) AIScreenshotAssi generates output and either pastes it back or shows it in the capsule, depending on the scenario
...
Set in `.env`:
- `AISCREENSHOTASSI_CTX_SCREENSHOT=1`
...
- `AISCREENSHOTASSI_CTX_FULLPAGE=1`
...
- `AISCREENSHOTASSI_CTX_SCROLL_CAPTURE=1`
 - Direction: `AISCREENSHOTASSI_CTX_SCROLL_DIRECTION=down|up|both`

### Dedicated vision model (recommended)

To keep the main model as a “black box” text model, AIScreenshotAssi can use a separate vision model to convert screenshots into a compact “screen context”, then feed that into the main model.

Example `.env` settings:
- `AISCREENSHOTASSI_VISION_EXTRACT=1`
- `AISCREENSHOTASSI_VISION_MODEL=gpt-5.2`
- `AISCREENSHOTASSI_VISION_API_KEY=...`
- `AISCREENSHOTASSI_VISION_BASE_URL=...`
